...
FloatNotes.prototype = {
	...
	if (event.originalTarget instanceof HTMLDocument) {
		...
		$(doc).bind('scroll', function(e) {
			...
			var notes = $('.floatnotes-note:visible', this).filter(function(){return $(this).data('id')}).each(function() {
				...
				if($above.find('#' + id).length == 0) {
				    $above.data('count', $above.data('count') + 1);
				    ele.appendTo($above_texts);
				}
				in_view = false;
			    }
			    else if(($(doc).scrollTop() + $(win).height()) < top) {
				if($below.find('#' + id).length == 0) {
				    $below.data('count', $below.data('count') + 1);
				    ele.appendTo($below_texts);
				}
				in_view = false;
			    }
			    if(in_view) {
				...
				else if($below.find('#' + id).length > 0){
				    $below.data('count', $below.data('count') - 1);
				}
				$('#' + id, doc).remove();
			    }
			});
			if($above.data('count') > 0) {
			    $above
			    .find('.floatnotes-label')
				.text($above.data('count') + ' note'  + (($above.data('count') > 1) ? 's': '') +  ' above')
			    .end()
			    .show().trigger('fade');
			}
			else {
			...
			if($below.data('count') > 0) {
			    $below
			    .find('.floatnotes-label')
				.text($below.data('count') + ' note' + (($below.data('count') > 1) ? 's': '') + ' below')
			    .end()
			    .show().trigger('fade');
			}
			else {
	...
	.find('.floatnotes-drag')
	    ...
	    .hide()
	.end()
	.find('.floatnotes-resize')
	    .hide()
	.end()
	.find('.floatnotes-content')
	    .bind('dblclick', {docs: doc}, function(event) { // dblick enables editing
	...
	.css({'width': data.w, 'height': data.h, 'top': data.y, 'left': data.x, 'position': 'absolute'})
        .jqResize('.floatnotes-resize', function(w, h) {
	...
	.bind({
		...
		$(doc.body).bind('click.floatnotes', {note: $note}, function(e) {
		    alert(1);
    ...
    getLocations: function(doc, search) {
	...
	for each (let l in loc) {
	    ...
	    item.setAttribute('checked', (this.contextNote.data('url') == l));
